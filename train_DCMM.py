...
def main(config: DictConfig):
    ...
    if config.task == "Tracking" and config.checkpoint_tracking:
        ...
        config.task == "Catching_TwoStage" or config.task == "Catching_OneStage"
    ...
    env = gym.make_vec(
        ...
        object_eval=config.object_eval,
        env_time=2.5,
        steps_per_policy=20,
    )
    ...
    PPO = (
        ...
        else PPO_Catch_TwoStage
        if config.task == "Catching_TwoStage"
        else PPO_Catch_OneStage
    )
