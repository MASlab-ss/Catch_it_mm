...
class PPO_Catch_TwoStage(object):
    def __init__(self, env, output_dif, full_config):
        ...
        self.episode_test_rewards = AverageScalarMeter(
            self.ppo_config["test_num_episodes"]
        )
    ...
    def load_tracking_model(self, checkpoint_tracking, checkpoint_catching):
        ...
        if checkpoint_catching or not checkpoint_tracking:
            return
        self.model.actor_mlp_t.load_state_dict(
            torch.load(checkpoint_tracking, map_location=self.device)["tracking_mlp"]
        )
        self.model.mu_t.load_state_dict(
            torch.load(checkpoint_tracking, map_location=self.device)["tracking_mu"]
        )
        self.model.sigma_t.data.copy_(
            torch.load(checkpoint_tracking, map_location=self.device)["tracking_sigma"]
        )
        print("self.model.sigma_t.data: ", self.model.sigma_t.data)
        self.running_mean_std_track.load_state_dict(
            torch.load(checkpoint_tracking, map_location=self.device)[
                "running_mean_std"
            ]
        )
        print("### Done loading tracking model")
    ...
    def train(self):
        ...
        while self.agent_steps < self.max_agent_steps:
            ...
            _last_t = time.time()
            info_string = (
                f"Agent Steps: {int(self.agent_steps // 1e3):04}K | FPS: {all_fps:.1f} | "
    ...
    def play_steps(self):
        for n in range(self.horizon_length):
            ...
            self.episode_success.update(
                torch.tensor(truncates, dtype=torch.float32, device=self.device)[
                    done_indices
                ]
            )
    ...
    def play_test_steps(self):
        for _ in range(self.horizon_length):
            ...
            self.episode_test_success.update(
                torch.tensor(truncates, dtype=torch.float32, device=self.device)[
                    done_indices
                ]
            )
