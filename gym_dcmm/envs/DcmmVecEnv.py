...
class DcmmVecEnv(gym.Env):
    ...
    def __init__(
        ...
        self.Dcmm = MJ_DCMM(
            viewer=viewer, object_name=object_name, object_eval=object_eval
        )
        ...
        self.hand_start_id = (
            mujoco.mj_name2id(self.Dcmm.model, mujoco.mjtObj.mjOBJ_GEOM, "piper_link7")
            - 1
        )
        # print("self.hand_start_id: ", self.hand_start_id)
        self.floor_id = mujoco.mj_name2id(
            self.Dcmm.model, mujoco.mjtObj.mjOBJ_GEOM, "floor"
        )
        self.object_id = mujoco.mj_name2id(
            self.Dcmm.model, mujoco.mjtObj.mjOBJ_GEOM, self.object_name
        )
        self.base_id = mujoco.mj_name2id(
            self.Dcmm.model, mujoco.mjtObj.mjOBJ_GEOM, "base_link"
        )
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "arm": spaces.Dict(
                    {
                        ...
                        "joint_pos": spaces.Box(
                            low=np.array(
                                [self.Dcmm.model.jnt_range[i][0] for i in range(5, 11)]
                            ),
                            high=np.array(
                                [self.Dcmm.model.jnt_range[i][1] for i in range(5, 11)]
                            ),
                            dtype=np.float32,
                        ),
                    }
                ...
                "hand": spaces.Box(
                    low=np.array(
                        [self.Dcmm.model.jnt_range[i][0] for i in range(11, 13)]
                    ),
                    high=np.array(
                        [self.Dcmm.model.jnt_range[i][1] for i in range(11, 13)]
                    ),
                    dtype=np.float32,
                ),
                "object": spaces.Dict(
    ...
    def _get_contacts(self):
        ...
        geom1_hand = np.where(
            (geom1_ids < self.object_id) & (geom1_ids >= self.hand_start_id)
        )[0]
        geom2_hand = np.where(
            (geom2_ids < self.object_id) & (geom2_ids >= self.hand_start_id)
        )[0]
    ...
    def _get_relative_ee_pos3d(self):
        ...
        return np.array(
            [
                x,
                y,
                self.Dcmm.data.body("piper_link6").xpos[2]
                - self.Dcmm.data.body("piper_link1").xpos[2],
            ]
        )
    ...
    def _get_relative_ee_quat(self):
        ...
        quat = relative_quaternion(
            self.Dcmm.data.body("base_link").xquat,
            self.Dcmm.data.body("piper_link6").xquat,
        )
    ...
    def _get_obs(self):
        ...
        obs = {
            ...
            "arm": {
                ...
                "joint_pos": np.array(self.Dcmm.data.qpos[15:21])
            ...
            "object": {
                ...
                "v_lin_3d": (obj_pos3d - self.prev_obj_pos3d) * self.fps
        ...
        if object_body is not None:
            ...
            if inertial is not None:
                ...
                self.random_mass = np.random.uniform(
                    DcmmCfg.object_mass[0], DcmmCfg.object_mass[0]
                )
            ...
            if joint is not None:
                ...
                random_damping = np.random.uniform(
                    DcmmCfg.object_damping[0], DcmmCfg.object_damping[1]
                )
    ...
    def random_object_pose(self):
        ...
        # low_factor = True
        if low_factor:
            height = 0.7 + 0.3 * np.random.rand()  # (0.7, 1.0)
        else:
            height = 1.0 + 0.6 * np.random.rand()  # (1.0, 1.6)
        # Random Velocity
        r_vel = 1 + np.random.rand()  # (1, 2)
        alpha_vel = math.pi * (
            np.random.rand() * 1 / 6 + 5 / 12
        )  # alpha_vel = (5/12 * pi, 7/12 * pi)
        ...
        # Random Static Time
        self.object_static_time = np.random.uniform(
            DcmmCfg.object_static[0], DcmmCfg.object_static[1]
        )
        # Random Quaternion
        r_obj_quat = R.from_euler(
            "xyz", [0, np.random.rand() * 1 * math.pi, 0], degrees=False
        )
    ...
    def random_PID(self):
        ...
        self.Dcmm.arm_pid.reset(
            self.k_arm * (DcmmCfg.k_arm[1] - DcmmCfg.k_arm[0]) + DcmmCfg.k_arm[0]
        )
        # self.Dcmm.steer_pid.reset(self.k_steer*(DcmmCfg.k_steer[1]-DcmmCfg.k_steer[0])+DcmmCfg.k_steer[0])
        self.Dcmm.drive_pid.reset(
            self.k_drive * (DcmmCfg.k_drive[1] - DcmmCfg.k_drive[0])
            + DcmmCfg.k_drive[0]
        )
        self.Dcmm.hand_pid.reset(
            self.k_hand[0] * (DcmmCfg.k_hand[1] - DcmmCfg.k_hand[0]) + DcmmCfg.k_hand[0]
        )
    ...
    def reset(self):
        ...
        ctrl_delay = np.array(
            [
                len(self.action_buffer["base"]),
                len(self.action_buffer["arm"]),
                len(self.action_buffer["hand"]),
            ]
        )
        info["ctrl_params"] = np.concatenate(
            (self.k_arm, self.k_drive, self.k_hand, ctrl_delay)
        )
    ...
    def norm_ctrl(self, ctrl, components):
        ...
        ctrl_array = np.concatenate(
            [
                ctrl[component] * DcmmCfg.reward_weights["r_ctrl"][component]
    ...
    def compute_reward(self, obs, info, ctrl):
        ...
        ) * DcmmCfg.reward_weights["r_base_pos"]
        reward_ee_pos = (
            self.info["ee_distance"] - info["ee_distance"]
        ) * DcmmCfg.reward_weights["r_ee_pos"]
        reward_ee_precision = (
            math.exp(-50 * info["ee_distance"] ** 2)
            * DcmmCfg.reward_weights["r_precision"]
        )
        ...
        if self.task == "Catching":
            ...
            if self.stage == "tracking":
                ...
                if self.reward_touch:
                    ...
                    ) * DcmmCfg.reward_weights["r_stability"]
                else:
    ...
    def step(self, action):
        ...
        if self.task == "Catching":
            if (
                info["ee_distance"] < DcmmCfg.distance_thresh
